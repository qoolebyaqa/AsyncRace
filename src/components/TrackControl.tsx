  ...
  return (
    <div className={styles.trackControl}>
      <div className={styles.raceButtons}>
        <button onClick={handleStartRace} disabled={btnDisabled}>
          RACE ▶
        </button>
        <button onClick={handleResetRace} disabled={!btnDisabled}>
          RESET ↻
        </button>
      </div>
      <TrackCarControlForm titleBtn="CREATE" />
      <TrackCarControlForm titleBtn="UPDATE" />
      <button onClick={handleGeneratingCars} disabled={btnDisabled}>
        GENERATE CARS
      </button>
    </div>
  );
