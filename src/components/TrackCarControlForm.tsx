...
function TrackCarControlForm({titleBtn}:{titleBtn:string}) {
  ...
  const selectedCar = useSelector((state:GlobalStateType) => state.Track.selectedCar);
  ...
  async function handleCarForm(event:React.FormEvent<HTMLFormElement>) {
    ...
    const carToAdd = {name: model.current!.value, color: color.current!.value}
    ...
    if(titleBtn === 'UPDATE' && model.current?.value !== '' && selectedCar) {
      const updatedCar = {id: selectedCar.id, name:model.current!.value, color:color.current!.value}
      await apiService.updateCar(updatedCar.id, updatedCar.name, updatedCar.color);
  ...
  <form className={styles.form} onSubmit={handleCarForm}>
    ...
    <input type="color" ref={color} defaultValue='#4682B4'/>
    <button disabled={!selectedCar && titleBtn === 'UPDATE'}>{titleBtn}</button>
  </form> );
}
