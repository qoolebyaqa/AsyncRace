...
function TrackCarControlForm({ titleBtn }: { titleBtn: string }) {
  ...
  const [colorOfPreview, setcolorOfPreview] = useState({visible: false, color: "#4682B4"});
  ...
  function handleChangeInput() {
    setcolorOfPreview((prev) => {
      return {...prev, visible: true, color: color.current!.value}
    })
  }
  return (
    <form className={styles.form} onSubmit={handleCarForm}>
      <input type="text" ref={model} disabled={!selectedCar && titleBtn === "UPDATE"}/>
      <input type="color" ref={color} defaultValue="#4682B4" onChange={titleBtn === "CREATE" ? handleChangeInput : undefined} disabled={!selectedCar && titleBtn === "UPDATE"}/>
