...
function Participant({ name, color, id }: ICarRes) {
  ...
  useEffect(() => {
    dispatch(
      trackActions.setDistance(
        finish.current!.offsetLeft - carElem.current!.offsetLeft,
      ),
    );
  }, []);
  ...
  async function handleStartEngine() {
    dispatch(
      trackActions.setRenderedCarsWithParams(
        await getCarParams(
          id,
          name,
          finish.current!.offsetLeft - carElem.current!.offsetLeft,
  ...
  async function endAnimation() {
    if (winner.length < 1) {
      ...
      dispatch(
        trackActions.setRaceWinner({
          name,
          duration: (newWinnerParams.duration / 1000).toFixed(2),
          color,
        }),
      );
      const winnerToCheck = winnersTable.filter(
        (winner) => winner.id === id,
      )[0];
      if (!winnerToCheck) {
        await apiService.createWinner({
          id,
          wins: 1,
          time: Number((newWinnerParams.duration / 1000).toFixed(2)),
        });
        dispatch(
          trackActions.pushToWinnersTable({
            id,
            name,
            duration: (newWinnerParams.duration / 1000).toFixed(2),
            wins: 1,
        ...
        if (
          dublicateFromAPI.time
          > Number((newWinnerParams.duration / 1000).toFixed(2))
        ) {
          await apiService.updateWinner({
            id,
            wins: winnerToCheck.wins + 1,
            time: Number((newWinnerParams.duration / 1000).toFixed(2)),
          });
        } else {
          await apiService.updateWinner({
  ...
  return (
    <div className={styles.participant}>
      <div className={styles.container}>
        ...
        <div className={styles.btnContainer}>
          <button
            ...
            disabled={carsInWayID.includes(id) || raceStarted}
          >
            A
          </button>
          <button
        ...
        <div
          ...
          style={
            carsInWayID.includes(id) || raceStarted
              ? carsParams.filter((animCar) => animCar.id === id)[0]
                .stylesForAnimation
              : { position: 'relative' }
          }
        >
      ...
      <div className={styles.trackFinish} ref={finish} />
    </div>
  );
}
